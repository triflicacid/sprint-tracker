...
describe("createSubtask", () => {
    it("creates a subtask in NEW status and records it in history", () => {
        const subtask = createSubtask(storyId, { description: "add endpoint" });
...
describe("getSubtaskById / getSubtasksForStory", () => {
    it("fetches a subtask by id", () => {
        const created = createSubtask(storyId, { description: "x" });
    ...
    it("lists every subtask for a story in creation order", () => {
        const a = createSubtask(storyId, { description: "a" });
        const b = createSubtask(storyId, { description: "b" });
        expect(getSubtasksForStory(storyId).map((s) => s.id)).toEqual([a.id, b.id]);
    });
});

describe("updateSubtask - plain field updates", () => {
    it("updates description without touching status or history", () => {
        const subtask = createSubtask(storyId, { description: "old" });
        const updated = updateSubtask(subtask.id, { description: "new" });
    ...
    it("updates complexity rating independently of status", () => {
        const subtask = createSubtask(storyId, { description: "x" });
    ...
    it("throws for a missing subtask", () => {
        expect(() => updateSubtask(999999, { description: "x" })).toThrow(SubtaskUpdateError);
    });
});
